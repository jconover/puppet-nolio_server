...
$package_source            = "",
$temp_dir                  = "/tmp",
$nolio_execution_name      = "",#<EXECUTION SERVER NAME OR IP> <- execution server to try connecting to
$nolio_execution_port      = 6600,
$nolio_product_key         = "",
$template_version          = "5x",

$nolio_db_type                          = "0", # 0=Oracle, 1=MS_SQL?
$sys_installationDir                    = "/opt/CA/Nolio", #Nolio 4ever
...
$nolio_repository_scheme_int            = 0,#0
 ...
 file {"${temp_dir}/puppet_nolio":
  ...
  exec { "${real_package_name}_download":
    command => "/usr/bin/wget -N '${package_source}'",
    cwd     => "${src_dir}",
